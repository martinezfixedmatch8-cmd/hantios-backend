...
Signup + Login (the full Auth Redesign — email/password, Google OAuth, email verification, refresh-token rotation with CSRF, remember-me, New-Device OTP) is **done**, 2026-07-20 — see "Auth: Signup + Login" under Auth Architecture below. Staff Invitation's tests were retrofitted to go through real `/auth/signup` → `/auth/login` instead of hand-minted JWTs (`tests/helpers/factories.ts`'s `signupTestOwner`/`loginTestOwner`). Next up, in order of what actually unblocks real usage:
